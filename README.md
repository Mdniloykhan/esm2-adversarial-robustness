...
├── esm2_runtime_results.csv             # Runtime measurements for defense mechanisms
...
Inference time for defense mechanisms measured on a single 76 AA sequence (Ubiquitin) on NVIDIA T4 GPU, averaged over 10 trials.
...
**Key finding:** Bayesian uncertainty estimation imposes the highest overhead (9.5× baseline) due to repeated dropout forward passes. Ensemble inference with 3 models is more practically deployable at 2.8× overhead.

Runtime data saved in `esm2_runtime_results.csv`. Runtime experiment code included in Step 11 of the notebook.
...
@article{niloy2024adversarial,
  title={Probing Adversarial Robustness of Protein Language Models: 
         A Reproducible Case Study of ESM-2 Under Substitution-Based Attacks},
